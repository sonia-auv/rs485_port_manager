...
RS485Driver* RS485Driver::_instance= nullptr;
    ...
    RS485Driver *RS485Driver::GetInstance(){
        if ( _instance == nullptr){
            _instance = new RS485Driver();
        }
        return _instance;
    }


    void RS485Driver::AddObservateur(rs485_port_manager::InterfaceModuleRS485* interfaceModule){
        ObservateurInterfaceModule.push_back(interfaceModule);
    }
    void RS485Driver::AddMessage(queueObject msg){
        _writerQueue.push_back(msg);
        _cvReaderWriter.notify_all();
    }

    void RS485Driver::Start() {
        _reader = std::thread(std::bind(&RS485Driver::readData, this));
        _writer = std::thread(std::bind(&RS485Driver::writeData, this));
        _parser = std::thread(std::bind(&RS485Driver::parseData, this));
    }

    bool RS485Driver::OpenPort()
    {
    ...
    void RS485Driver::messageRS485CallBack(const sonia_common_ros2::msg::RS485msg &msg)
    {
        queueObject ser;
        ser.cmd = msg.cmd;
        ser.slave = msg.slave;
    ...
    std::tuple<uint8_t, uint8_t> RS485Driver::checkSum(uint8_t slave, uint8_t cmd, uint8_t nbByte,
    ...
    void RS485Driver::parseData()
    {
        ...
        while (_thread_control)
        {
            ...
            else
            {
                ...
                {
                    for(size_t i =0; i< ObservateurInterfaceModule.size();++i){
